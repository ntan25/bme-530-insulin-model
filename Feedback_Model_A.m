function dydt = Feedback_Model_A(t,y)
...
%         x1 = 10 * abs(sin(t));
    ...
    n = 4; 
    tau = 1.5; 
    PKC = (Vmax * x19 * (t-tau)) / (Kd^n + x19*(t-tau)^n); 

    dydt(1, 1) = k_1 * x3 + k_3 * PTP * x5 - k1 * x1 * x2 + k_4 * x6 - k4 * x2; 
    dydt(2, 1) = k1 * x1 * x2 - k_1 * x3 - k3 * x3; 
    dydt(3, 1) = k2 * x1 * x5 - k_2 * x4 + k_4p * x7 - k4p * x4; 
    dydt(4, 1) = k3 * x3 + k_2 * x4 - k2 * x1 * x5 - k_3 * PTP * x5 + k_4p * x8 - k4p * x5;
